...
{
    // Remove autogenerated rituals for Mousekin ideologies, leaving only explicitly-defined ones
    ...
    public static class Harmony_IdeoFoundation_AddSpecialPrecepts_RemoveAutogeneratedRituals
    {
        static void Postfix(ref IdeoFoundation __instance)
        {
            if (__instance.ideo.culture.IsMousekin())
            {
                ...
                __instance.ideo.precepts.RemoveAll(p => p is Precept_Ritual && !hardcodedRitualPrecepts.Contains(p.def));
